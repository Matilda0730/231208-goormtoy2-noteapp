...
import { toggleLabel } from "reduxprops/features/selectedLabels/selectedLabelsSlice";
...
const MemoLabelModal = () => {
  ...
  const existLabelSpace = useSelector(
    (state: RootState) => state.menu.newLabelSpace
  );
  const [selectedLabels, setSelectedLabels] = useState<{
    [key: string]: boolean;
  }>({}); // 선택된 라벨 상태
  const isLabelModalOpen = useSelector(
  ...
  const handleLabelSearchChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setLabelSearch(event.target.value);
  };
  const selectedLabelsState = useSelector(
    (state: RootState) => state.selectedLabels.selectedLabels
  );
  ...
  //라벨 뿌려주기 위해서 선택해오기
  const newLabelSpace = useSelector(
    (state: RootState) => state.menu.newLabelSpace
  );
    ...
    <div onClick={handleModalContainerClick} ref={modalRef}>
      ...
      <div className={styles.modal_name_container}>
        <form
          ...
          className={styles.labelAndSearches_container}
        >
          <input
            type="text"
          ...
          {filteredLabels.map((label) => (
            <div key={label.id} className={styles.label_item}>
              <input
                ...
                checked={selectedLabelsState[label.id] || false}
              ...
              >
                ...
                <div className={styles.label_name}>{label.name}</div>
              </label>
            </div>
          ))}
          <div>
