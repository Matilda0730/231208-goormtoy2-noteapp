...
import { clearSelectedLabels } from "reduxprops/features/selectedLabels/selectedLabelsSlice";
...
const CreateMemo = () => {
	...
	const selectedLabelsState = useSelector((state: RootState) => state.selectedLabels);
	const labels = useSelector((state: RootState) => state.menu.newLabelSpace);

	// 선택된 라벨 이름 배열을 가져오는 함수
	const selectedLabelObjects = useMemo(() => {
		return Object.keys(selectedLabelsState.selectedLabels)
			.filter((labelId) => selectedLabelsState.selectedLabels[labelId])
	...
	const handleClickOutside = (event: globalThis.MouseEvent) => {
		if (
			...
			if (!title && !text) {
				// 제목과 내용이 모두 비어있는 경우, 선택된 라벨을 초기화
	...
	useEffect(() => {
		if (!isVisible) {
			...
			dispatch(clearSelectedLabels()); // 모달 닫힐 때 라벨 선택 상태 초기화
		}
	}, [isVisible, dispatch]);
			...
			{isVisible ? (
				<>
					...
					>
						...
						<div className={styles.selected_labels_container}>
							{selectedLabelObjects.map((label, index) => (
								<div key={index} className={styles.selected_label}>
									{label.name}
								</div>
							))}
						</div>
