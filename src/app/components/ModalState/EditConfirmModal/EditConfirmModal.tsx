  ...
  return (
    ...
    >
      <div className={styles.text_space}>
        {`'${mergeToExisted?.name}'라벨을 '${existedLabel?.name}'라벨과 병합하시겠습니까? 라벨이 '${mergeToExisted?.name}'(으)로 지정된 모든 메모는 '${existedLabel?.name}'(으)로 라벨이 지정되며 '${mergeToExisted?.name}' 라벨은 삭제됩니다.`}
      </div>
