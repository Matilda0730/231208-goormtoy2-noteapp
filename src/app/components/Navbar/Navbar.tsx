...
const Navbar: React.FC = () => {
  ...
  const labelDeleted = useSelector(
    (state: RootState) => state.menu.labelDeleted
  );
  const existedLabel = useSelector(
    (state: RootState) => state.menu.existedLabel
  );
  const mergeFlag = useSelector((state: RootState) => state.menu.mergeFlag);
  ...
  useEffect(() => {
    if (labelDeleted && pathname.startsWith("/pages/label/")) {
      if (mergeFlag) {
        dispatch(resetLabelDeleted());
        dispatch(setMergeFlag(false));

        dispatch(setSelectedMenu(existedLabel?.id));
        push(`/pages/label/${existedLabel?.name}`);
        return;
      }
      // 리디렉션 전에 상태 리셋
      dispatch(resetLabelDeleted());
      ...
      dispatch(setSelectedMenu("memo"));
      push("/");
    }
  }, [labelDeleted, pathname, dispatch, push]);
